<?php
	...
	while($row = $purchaseDetailsSearchStatement->fetch(PDO::FETCH_ASSOC)){
		...
		$output .= '<tr>' .
						...
						'<td>' . $row['vendorID'] . '</td>' .
						'<td>' . $totalPrice . '</td>' .
						'<td>' . '<a href="generate_bill2.php?purchaseID='.$row['purchaseID'].'"><img style=" width: 50px; /* adjust the width to your desired size */
						height: 50px; " src="data\item_images\pdf_icon.png" alt="PDF"></a>' . '</td>' .
					'</tr>';
	}
